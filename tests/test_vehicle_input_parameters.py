...
def test_can_pass_directly():
    ...
    e.remove("foobazzle")
    assert len(vip.CarInputParameters(d, e).powertrains) == 5
    assert len(vip.CarInputParameters(d, e).parameters) == 12


def test_alternate_filepath():
    assert len(vip.CarInputParameters(DEFAULT, EXTRA).powertrains) == 5
    assert len(vip.CarInputParameters(DEFAULT, EXTRA).parameters) == 13
